    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 py-12">
      ...
      >
        <div class="flex justify-center items-center bg-gray-100 h-40 overflow-hidden">
          <img v-if="item.image" :src="item.image" alt="Gambar Artikel" class="object-cover w-full h-40 group-hover:scale-105 transition-transform duration-300" />
        ...
        <div class="p-6 flex-1 flex flex-col">
          ...
          <div class="flex justify-center mb-2 gap-2 flex-wrap">
            ...
            <span class="px-2 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700 flex items-center"><i class="fas fa-calendar mr-1"></i>{{ formatDate(item.published_at) }}</span>
          </div>
          <p v-if="item.content" class="text-gray-600 text-xs mt-2 text-center line-clamp-2" v-html="item.content"></p>
        </div>
      </div>
    </div>
    <!-- Modal Detail -->
        ...
        <div class="bg-white rounded-lg shadow-lg max-w-lg w-full p-6 relative animate-fadeIn">
          ...
          <div class="flex justify-center mb-4">
            <div class="w-full h-56 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
              <img v-if="selected.image" :src="selected.image" alt="Gambar Artikel" class="object-cover w-full h-56" />
              <div v-else class="flex flex-col items-center justify-center w-full h-full text-gray-300">
                <i class="fas fa-newspaper text-4xl mb-2"></i>
                <span class="text-xs">Tidak ada gambar</span>
              </div>
          ...
          <div class="flex justify-center mb-2 gap-2 flex-wrap">
            ...
            <span class="px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700 flex items-center"><i class="fas fa-calendar mr-1"></i>{{ formatDate(selected.published_at) }}</span>
          </div>
          <div class="mb-2 flex items-center text-gray-600"><i class="fas fa-align-left mr-2"></i> <span class="font-semibold">Konten:</span> <span v-html="selected.content"></span></div>
        </div>
      </div>
    </transition>
  </div>
...
<script setup>
import { ref } from 'vue';
const props = defineProps({ artikelData: Array });
